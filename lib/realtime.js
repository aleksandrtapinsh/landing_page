...
/** Wipe the log and tell every open client to clear what it is showing. */
async function clearChat() {
  const { deletedCount } = await db.messages().deleteMany({});
  // Nothing was there, so nobody can be looking at it — stay quiet rather than
  // print "chat was cleared" into an already-empty log.
  if (deletedCount > 0) broadcast({ type: 'cleared' });
  return deletedCount;
}
...
module.exports = { attach, listChatters, notifyUser, clearChat, PATH };
