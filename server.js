...
db.connect()
  .then(async () => {
    ...
    // Chat does not survive a restart: the log starts empty with the app.
    // (Messages also expire individually 12h after being sent — see db.js.)
    const { deletedCount } = await db.messages().deleteMany({});
    if (deletedCount > 0) {
      console.log(`[chat] cleared ${deletedCount} message${deletedCount === 1 ? '' : 's'} on startup`);
    }
