// MongoDB connection and collection handles.
//
// The database is created on first write; all this does is connect and make
// sure the indexes that enforce uniqueness and expiry exist.

const { MongoClient } = require('mongodb');

// Read lazily rather than at module load, so it doesn't matter whether the
// caller loaded its .env before or after requiring this file.
const uri = () => process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017';
const dbName = () => process.env.MONGODB_DB || 'sasha_site';

let client = null;
let db = null;

async function connect() {
  if (db) return db;

  client = new MongoClient(uri(), { serverSelectionTimeoutMS: 5000 });
  await client.connect();
  db = client.db(dbName());

  // Uniqueness is enforced here rather than by checking before insert, so two
  // simultaneous signups can't both pass the check and both succeed.
  await db.collection('users').createIndexes([
    { key: { email: 1 }, unique: true },
    { key: { usernameLower: 1 }, unique: true },
  ]);

  // Mongo drops session documents itself once expiresAt passes.
  await db.collection('sessions').createIndexes([
    { key: { expiresAt: 1 }, expireAfterSeconds: 0 },
  ]);

  // Every message lives exactly this long after being sent, live or not;
  // Mongo's TTL sweep (runs about once a minute) removes them by itself.
  const MESSAGE_TTL_SECONDS = 60 * 60 * 12;
  try {
    await db.collection('messages').createIndexes([
      { key: { createdAt: -1 } },
      { key: { createdAt: 1 }, expireAfterSeconds: MESSAGE_TTL_SECONDS },
    ]);
  } catch (err) {
    // A TTL index can't be redefined by createIndexes: databases created
    // before the lifetime changed (it used to be 30 days) answer with
    // IndexOptionsConflict. Adjust the existing index in place instead.
    if (err.code !== 85) throw err;
    await db.command({
      collMod: 'messages',
      index: { keyPattern: { createdAt: 1 }, expireAfterSeconds: MESSAGE_TTL_SECONDS },
    });
  }

  // Leftover from the retired "wipe after 12h offline" sweep clock.
  await db.collection('meta').drop().catch(() => {});

  return db;
}

async function close() {
  await client?.close();
  client = null;
  db = null;
}

const users = () => db.collection('users');
const sessions = () => db.collection('sessions');
const messages = () => db.collection('messages');

module.exports = { connect, close, users, sessions, messages, uri, dbName };
